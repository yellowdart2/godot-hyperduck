...
func _physics_process(_delta):
	# make the walls move for this level only
	var left_wall = get_tree().get_nodes_in_group("left_wall")
	for node in left_wall:
		node.position.x += .5
	var right_wall = get_tree().get_nodes_in_group("right_wall")
	for node in right_wall:
		node.position.x -= .5
...
func reset_player():
	...
	get_tree().change_scene_to_file.bind("res://levels/level_013.tscn").call_deferred()
...
func _on_player_reached_door():
	if has_key == true:
		...
		has_key = false
		get_tree().change_scene_to_file.bind("res://levels/level_014.tscn").call_deferred()
