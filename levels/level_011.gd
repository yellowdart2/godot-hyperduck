...
func _physics_process(_delta):
	...
	if player.position.y > 165:
		reset_player()

func reset_player():
	player.position.x = reset_player_x
	player.position.y = reset_player_y 
	get_tree().change_scene_to_file.bind("res://levels/level_011.tscn").call_deferred()
...
func _on_player_reached_door():
	if has_key == true:
		...
		has_key = false
		get_tree().change_scene_to_file.bind("res://levels/level_012.tscn").call_deferred()
